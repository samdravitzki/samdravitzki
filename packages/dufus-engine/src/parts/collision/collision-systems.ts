...
import { Aabb, Circle } from "./intersection/intersection-shapes";
...
function calculateIntersection(
  positionA: Vector,
  colliderA: ColliderData,
  positionB: Vector,
  colliderB: ColliderData,
) {
  ...
  if (colliderA.type === "circle" && colliderB.type === "circle") {
    ...
    return cirlceCirlceIntersection(circle1, circle2);
  }
  ...
  for (const [entityA, positionA, colliderA] of colliders) {
    for (const [entityB, positionB, colliderB] of colliders) {
      ...
      const intersection = calculateIntersection(
        positionA.componentData.position,
        colliderA.componentData,
        positionB.componentData.position,
        colliderB.componentData,
      );
